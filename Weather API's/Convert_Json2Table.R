...
dailySummary <- function(filename){
  ...
  for(i in 1:length(output$history$dailysummary))
  {
    ...
    b <- cbind(b,as.character(date["pretty",1]))
    b <- cbind(b,as.character(date["tzname",1]))
    b <- cbind(b,output$history$dailysummary[[i]]$fog)
    b <- cbind(b,output$history$dailysummary[[i]]$rain)
    b <- cbind(b,output$history$dailysummary[[i]]$snow)
    b <- cbind(b,output$history$dailysummary[[i]]$snowfallm)
    b <- cbind(b,output$history$dailysummary[[i]]$snowfalli)
    writeLines("===============================================================================")
